...
class InsuranceRepository
{
    ...
    public function save(array $data): int
    {
        if (isset($data['id'])) {
            $id = $data['id'];
            unset($data['id']);
            $this->db->update('vm_vehicle_insurance', $id, $data);
            return $id;
        } else {
            return $this->db->insert('vm_vehicle_insurance', $data);
        }
    }

    public function delete(int $id): bool
    {
        return $this->db->delete('vm_vehicle_insurance', $id);
    }
}
