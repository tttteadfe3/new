document.addEventListener('DOMContentLoaded', () => {
    ...
    const ORG_API_URL = '/api/organization';
    ...
    const adjustmentModalEl = document.getElementById('adjustment-modal');
    ...
    const renderTable = () => {
        ...
        employeeDataStore.forEach(emp => {
            ...
            let rowClass = '';
            if (isCalculated) {
                statusBadge = '<span class="badge bg-info">계산됨</span>';
                rowClass = 'table-info';
            } else if (isGranted) {
                statusBadge = `<span class="badge bg-success">부여됨</span>`;
            } else {
                statusBadge = `<span class="badge bg-secondary">미부여</span>`;
            }
    ...
    const loadEntitlements = async () => {
        ...
        tableBody.innerHTML = `<tr><td colspan="11" class="text-center"><span class="spinner-border spinner-border-sm"></span> 목록 로딩 중...</td></tr>`;

        try {
            const year = yearFilter.value;
            const departmentId = departmentFilter.value;
            let url = `${API_BASE_URL}/entitlements?year=${year}`;
    ...
    const loadDepartments = async () => {
        try {
            // Assuming the organization API provides a list of departments
            const response = await fetch(`${ORG_API_URL}?type=department`, fetchOptions());
            const result = await response.json();
            if (!result.success) throw new Error(result.message);
    ...
    calculateBtn.addEventListener('click', async () => {
        ...
        try {
            const response = await fetch(`${API_BASE_URL}/calculate`, fetchOptions({
                method: 'POST',
                body: JSON.stringify({ year: yearFilter.value, department_id: departmentFilter.value })
            }));
            const result = await response.json();
            if (!result.success) throw new Error(result.message);
    ...
    saveBtn.addEventListener('click', async () => {
        ...
        try {
            const response = await fetch(`${API_BASE_URL}/save-entitlements`, fetchOptions({
                method: 'POST',
                body: JSON.stringify({
    ...
    if (adjustmentForm) {
        ...
        adjustmentForm.addEventListener('submit', async (e) => {
            ...
            try {
                const response = await fetch(`${API_BASE_URL}/adjust`, fetchOptions({
    ...
    yearFilter.addEventListener('change', loadEntitlements);
    departmentFilter.addEventListener('change', loadEntitlements);

    const init = async () => {
        // The department loading depends on Organization API, which might need to be refactored first.
