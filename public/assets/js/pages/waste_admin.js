document.addEventListener('DOMContentLoaded', () => {
    class WasteAdminApp {
        constructor() {
            this.config = { API_URL: '/api/waste-collections/admin' };
        ...
        async loadCollections() {
            ...
            try {
                const response = await this._fetch(`${this.config.API_URL}?${params}`);
        ...
        handleTableEvents(e) {
            ...
            if (target.classList.contains('btn-save-items')) {
                ...
                this.handleProcessCollection(id);
            }
        }
        ...
        async handleFileUpload(event) {
            ...
            try {
                const response = await this._fetch(`${this.config.API_URL}/parse-html`, { method: 'POST', body: formData });
        ...
        async handleBatchRegistration() {
            ...
            if (collections.length === 0) return Toast.error('등록할 데이터가 없습니다.');

            try {
                const response = await this._fetch(`${this.config.API_URL}/batch-register`, { method: 'POST', body: JSON.stringify({ collections }) });
        ...
        async saveItems(id, itemsJson) {
            try {
                await this._fetch(`${this.config.API_URL}/${id}/items`, { method: 'PUT', body: JSON.stringify({ items: itemsJson }) });
        ...
        async handleProcessCollection(id) {
            const result = await Confirm.fire('이 항목을 처리완료로 변경하시겠습니까?');
            if (result.isConfirmed) {
                try {
                    await this._fetch(`${this.config.API_URL}/${id}/process`, { method: 'POST' });
        ...
        async handleClearInternet() {
            ...
            if (result.isConfirmed) {
                try {
                    await this._fetch(`${this.config.API_URL}/online-submissions`, { method: 'DELETE' });
