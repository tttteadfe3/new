document.addEventListener('DOMContentLoaded', () => {
    ...
    const loadHistory = async () => {
        ...
        entitlementSummary.innerHTML = '불러오는 중...';
        leaveHistoryBody.innerHTML = `<tr><td colspan="6" class="text-center">불러오는 중...</td></tr>`;

        try {
            const response = await fetch(`../api/leaves_admin.php?action=get_history&employee_id=${employeeId}&year=${year}`, fetchOptions());
    ...
    const renderHistory = (data) => {
        ...
        if (data.entitlement) {
            ...
            entitlementSummary.innerHTML = `
                ...
                <span class="fw-bold ${remaining_days < 0 ? 'text-danger' : 'text-primary'}">${remaining_days}</span>일 남음
            `;
        } else {
