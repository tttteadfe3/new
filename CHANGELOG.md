...
### 🐛 버그 수정 (Bug Fixes)
- **사용자 역할 업데이트 시 SQL 오류 수정**:
  - **문제**: 사용자에게 여러 역할을 동시에 할당할 때 `SQLSTATE[HY093]: Invalid parameter number` 오류 발생.
  - **원인**: `UserRepository::updateUserRoles` 메서드에서 여러 역할을 한 번에 삽입하기 위해 동적으로 생성된 SQL 쿼리의 파라미터 바인딩 수가 플레이스홀더 수와 일치하지 않았음.
  - **수정**: 동적 쿼리 생성을 제거하고, 트랜잭션 내에서 각 역할을 개별적으로 `INSERT` 하도록 로직을 변경하여 파라미터 불일치 문제를 해결.
  - **영향 범위**: `app/Repositories/UserRepository.php`
  - **함께 수정된 파일**: 없음
